...
from VP_code.models.mambair_arch import MambaIR
from VP_code.models.DCN import DCNv2PackFlowGuided
...
class Gated_Aggregation(nn.Module):
    ...
    def __init__(self, hidden_channels, kernel_size, stride=1, padding=0, dilation=1, bias=True, activation=torch.nn.LeakyReLU(0.2, inplace=True)):
        ...
        self.conv2d_1 = nn.Sequential(torch.nn.Conv2d(in_channels=hidden_channels+hidden_channels+2, out_channels=hidden_channels+hidden_channels+2, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation, bias=bias),
                                      torch.nn.LeakyReLU(0.2, inplace=True),
                                      torch.nn.Conv2d(in_channels=hidden_channels + hidden_channels + 2,
                                                      out_channels=int(hidden_channels / 2), kernel_size=kernel_size,
        ...
        self.align = DCNv2PackFlowGuided(hidden_channels, hidden_channels, kernel_size=3, stride=1, padding=1, deformable_groups=1,
                                         pa_frames=2)
    ...
    def forward(self, hidden_state, curr_lr, residual_indicator, hidden_state_o=None, flow=None, pre_mask=None,  head = False, return_mask = False, return_feat = False):

        latent_feature = self.conv2d_projection_head(curr_lr)
        if hidden_state_o is not None:
            hidden_state = self.align(hidden_state_o, hidden_state, latent_feature, flow)
        x = self.activation(self.conv2d_1(torch.cat([hidden_state, latent_feature, residual_indicator, pre_mask], dim=1)))
...
class Video_Backbone(nn.Module):
    ...
    def __init__(self, num_feat=16, num_block=6, spynet_path=None):
        ...
        self.forward_resblocks = MambaIR(embed_dim=64, depths=[2, 2, 2], d_state=16, mlp_ratio=1.2, in_chans=num_feat, drop_rate=0.)
        self.backward_resblocks = MambaIR(embed_dim=64, depths=[2, 2, 2], d_state=16, mlp_ratio=1.2, in_chans=num_feat, drop_rate=0.)
    ...
    def forward(self, lrs):
        ...
        pre_mask = torch.ones_like(residual_indicator)
        for i in range(t-1, -1, -1):
            curr_lr = lrs[:, i, :, :, :]
            if i < t-1:
                flow = backward_flow[:, i, :, :, :]
                feat_prop_o = feat_prop
                feat_prop = flow_warp(feat_prop, flow.permute(0, 2, 3, 1)) # 1 h w 2
                ...
                feat_prop, pre_mask = self.Backward_Aggregation(feat_prop, curr_lr, residual_indicator, feat_prop_o, flow, pre_mask=pre_mask, head=False)
                feat_prop = self.backward_resblocks(feat_prop)
            else:
                feat_prop, pre_mask = self.Backward_Aggregation(feat_prop, curr_lr, residual_indicator, pre_mask=pre_mask, head=True)
                feat_prop = self.backward_resblocks(feat_prop)
        ...
        for i in range(0, t):
            ...
            if i > 0:
                ...
                feat_prop, pre_mask = self.Forward_Aggregation(feat_prop, curr_lr, residual_indicator, feat_prop_o, flow, pre_mask=pre_mask, head=False)
                feat_prop = self.forward_resblocks(feat_prop)
            else:
                feat_prop, pre_mask = self.Forward_Aggregation(feat_prop, curr_lr, residual_indicator, pre_mask=pre_mask, head=True)
                feat_prop = self.forward_resblocks(feat_prop)
