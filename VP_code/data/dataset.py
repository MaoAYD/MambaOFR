...
class Film_dataset_1x(data.Dataset):
    def __init__(self, root_dir):
        super(Film_dataset_1x, self).__init__()
        """
        初始化数据集类。

        :param root_dir: 数据集的根目录，每个子文件夹包含一个视频序列的图像帧。
        """
        self.root_dir = root_dir
        self.sequence_paths = []
        self.video_name = root_dir.replace("\\","/").split("/")[-1]
        for d in os.listdir(root_dir):
            v = os.path.join(root_dir, d)
            for k in os.listdir(v):
                self.sequence_paths.append(os.path.join(v, k, "frames").replace("\\","/"))

        self.trans = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])


    def load_sequence(self, sequence_path, grayscale=True):
        """
        加载指定索引的视频序列。

        :param index: 视频序列的索引。
        :param grayscale: 是否将图像转换为灰度，默认为True。
        :return: 包含视频序列中所有图像的列表。
        """
        images = []
        for image_name in sorted(os.listdir(sequence_path)):
            image_path = os.path.join(sequence_path, image_name)
            image = Image.open(image_path)
            if grayscale:
                image = image.convert('L').convert("RGB")  # 转换为灰度图像
                # image = image.resize((640, 368))
            images.append(self.trans(image))
        return torch.stack(images, dim=0)

    def __getitem__(self, index):
        s_name = self.sequence_paths[index]
        img_lqs = self.load_sequence(s_name)
        new_clip_sequence = sorted(os.listdir(os.path.join(s_name)))
        keys = s_name.split("/")[-3:]
        print(keys)
        key = os.path.join(keys[0], keys[1], keys[2])

        return {'lq': img_lqs, 'gt': img_lqs, 'key': key, 'frame_list': new_clip_sequence,
                'video_name': self.video_name, 'name_list': new_clip_sequence}
...
class Film_dataset_4(data.Dataset): ## 4 for REDS dataset + resize by 2
    ...
    def __getitem__(self, index):
        ...
        img_results = img2tensor(img_lqs, gray=True) ## List of tensor
        ...
        # img_gt: (t, c, h, w)
        # key: str
        (t, c, h, w) = img_lqs.shape
        # print(t, c, h, w, self.num_frame)
        if t == 1:
            img_lqs = img_lqs.squeeze(0)
            img_gts = img_gts.squeeze(0)
        elif t > 1 and c==1:
            img_lqs = img_lqs.squeeze(1)
            img_gts = img_gts.squeeze(1)            
        return {'lq': img_lqs, 'gt': img_gts, 'key': key, 'frame_list': frame_list, 'video_name': os.path.basename(current_lq_root)}
