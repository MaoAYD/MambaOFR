...
from VP_code.data.dataset import Film_dataset_1x
from VP_code.utils.util import worker_set_seed, get_root_logger, set_device, frame_to_video, frame_to_video_only
...
def Load_dataset(opts, config_dict):
    val_dataset = Film_dataset_1x(config_dict['datasets']['val']['dataroot_lq'])
...
def pad_video_to_64_multiple_with_zero_padding(video_tensor):
    _, frames, channels, height, width = video_tensor.shape

    # 计算目标尺寸
    target_height = ((height + 63) // 64) * 64
    target_width = ((width + 63) // 64) * 64
    ...
    # 使用零填充
    padded_video = F.pad(video_tensor, (pad_left, pad_right, pad_top, pad_bottom), mode='constant', value=0)

    return padded_video, (pad_top, pad_bottom, pad_left, pad_right)


def unpad_video(padded_video, padding_info):
    pad_top, pad_bottom, pad_left, pad_right = padding_info
    _, frames, channels, height, width = padded_video.shape
    ...
    # 移除padding
    unpadded_video = padded_video[:, :, :, pad_top:pad_top + original_height, pad_left:pad_left + original_width]
...
def validation(opts, config_dict, loaded_model, val_loader):
    ...
    for val_data in val_loader:  ### Once load all frames
        ...
        for i in range(0, all_len, opts.temporal_stride):
            ...
            part_lq, pad_info = pad_video_to_64_multiple_with_zero_padding(part_lq)
            with torch.no_grad():
                part_output = loaded_model(part_lq)
                part_output = unpad_video(part_output, pad_info)
        ...
        for id, sr_img in enumerate(sr_imgs):
            # print(test_clip_par_folder, clip_name, frame_name_list[id][0])
        ...
        frame_to_video_only(restored_clip_url, video_save_url)
        ###
        ...
        if calculate_metric:
            ...
            LPIPS_this_video = [loss_fn_alex(sr.cuda(), gt.cuda(), normalize=True).item() for sr, gt in
                                zip(val_output, gt)]
            PSNR += sum(PSNR_this_video) / len(PSNR_this_video)
            SSIM += sum(SSIM_this_video) / len(SSIM_this_video)
            LPIPS += sum(LPIPS_this_video) / len(LPIPS_this_video)
            print(LPIPS)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', type=str, default='RNN_Swin_4', help='The name of adopted model')
...
# python VP_code/test2.py --name RNN_Swin_4 --model_name RNN_Swin_4 --which_iter 200000 --temporal_length 20 --temporal_stride 10 --input_video_url E:\oldfilm --gt_video_url E:\oldfilm
