...
class Trainer():
    ...
    def optimize_parameters(self, lq, gt, current_iter):
        ...
        if self.opts.global_rank==0 and current_iter % self.config['train_visualization_iter'] == 0:  # Save the training samples
            print('saving image', os.path.join(self.config['path']['experiments_root'],'training_show_%s.png'%(current_iter)))
    ...
    def train(self):
        ...
        print(self.opts.global_rank, self.config['val']['val_freq'], self.config['logger']['save_checkpoint_freq'])
        import tqdm
