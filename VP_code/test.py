...
def validation(opts, config_dict, loaded_model, val_loader):
    ...
    for val_data in val_loader:  ### Once load all frames
        ...
        if calculate_metric:
            ...
            LPIPS_this_video = [loss_fn_alex(sr.cuda(),gt.cuda(), normalize=True).item() for sr,gt in zip(val_output, gt)]
...
# python VP_code/test.py --name RNN_Swin_4 --model_name RNN_Swin_4 --which_iter 200000 --temporal_length 20 --temporal_stride 10 --input_video_url E:\REDS\val\val_bu\de2 --gt_video_url E:\REDS\val\val_bu\gt2
